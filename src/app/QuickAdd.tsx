...
/* Последний использованный счёт — отдельно для расхода, дохода и перевода.
 * Сохраняется при успешном проведении операции. */
const LS_KEY = "moneta-last-acc-";

function readLastAcc(key: string): string | null {
  try {
    return window.localStorage.getItem(LS_KEY + key);
  } catch {
    return null;
  }
}

function saveLastAcc(key: string, id: string) {
  try {
    window.localStorage.setItem(LS_KEY + key, id);
  } catch {
...
export default function QuickAdd({
  ...
  const [accountId, setAccountId] = useState(() => {
    if (editTx) return editTx.accountId;
    // последний использованный счёт для этого типа операции
    const saved = readLastAcc("EXPENSE");
    if (saved && state.accounts.some((a) => a.id === saved)) return saved;
  ...
  // при смене типа: подставляем последний использованный счёт этого типа
  // и сбрасываем неподходящую категорию (первый рендер пропускаем —
  // иначе при редактировании терялась бы категория операции)
  ...
  useEffect(() => {
    ...
    if (!editTx) {
      const saved = readLastAcc(type);
      if (saved && state.accounts.some((a) => a.id === saved)) setAccountId(saved);
    }
    setCategoryId("");
    setErr(null);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    if (type !== "TRANSFER") return;
    if (!toOptions.some((a) => a.id === toAccountId)) {
      const saved = readLastAcc("TRANSFER_TO");
      const candidate = toOptions.some((a) => a.id === saved) ? (saved as string) : toOptions[0]?.id ?? "";
      setToAccountId(candidate);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [type, accountId]);
  ...
  const submit = () => {
    ...
    // запоминаем счёт — в следующий раз подставится сразу
    saveLastAcc(type, accountId);
    if (type === "TRANSFER") saveLastAcc("TRANSFER_TO", toAccountId);
    try { navigator.vibrate?.(12); } catch { /* нет вибро */ }
    const cur = accountsById[accountId]?.currency;
    toast.push(
      editTx
        ? "Операция обновлена"
        : type === "TRANSFER"
          ? `Перевод ${fmtMoney(amt, { currency: cur })} выполнен`
          : type === "INCOME"
            ? `Доход ${fmtMoney(amt, { currency: cur, sign: true })} добавлен`
            : `Расход ${fmtMoney(amt, { currency: cur })} добавлен`,
      "ok"
    );
    onClose();
  };
      ...
      >
        ...
        <div className="flex gap-2 pt-1">
          ...
          >
            ...
            {(() => {
              const amt = parseAmount(amount);
              return Number.isFinite(amt) && amt > 0 ? ` · ${fmtMoney(amt, { currency: acc?.currency })}` : "";
            })()}
          </button>
        </div>
      </form>
